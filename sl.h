...
bool Sl::readItem(vector<ll> &keys)
{
    ...
    // cache hit，遍历keys中的每个key
    for (int i = 0; i < keys.size(); i++)
    {
        //先检查读缓存中是否命中，再检查写缓存中是否命中
        if (isCached(keys[i]))
        {
            // 统计命中数
            st.read_hit_nums += 1;
            //accessKey调研Put更新LRU，读操作对读缓存进行不需要修改
    ...
    for (int i = 0; i < keys.size(); i++)
    {
        ...
        {
            isTraceHit = false;
            //读缓存和写缓存未命中，accessKey调用Put更新读缓存LRU，当前数据进读缓存
...
//isCache修改为int类型，1表示读缓存，2表示写缓存命中
void Sl::odirectWrite(bool isCache, const long long &offset, const long long &size)
{
    ...
    //定义全局变量，记录之前的动作、上一次I/O的时间、当前I/O时间、上上次I/O时间、轮次
    //如果SMR不允许写入，根据read操作缓存满且未命中的情况逐出，然后break
    // else
    // {
    //     // cout << "cache full" << endl;
    //     ll victim = getVictim(); // [lirs] ll victim = cache_map.getCurVictim();
...
void Sl::readCache(const ll &offset_cache)
{
    ...
    //读缓存命中时cached=1， 写缓存命中时cached=2，readCache多一个参数isReadCache，readItem处传入
    // if (isReadCache) {
