...
protected:
    ...
    void remove(const ll &key,int isReadCache);
...
void Sl::remove(const ll &key,int isReadCache)
{
    ...
    if (isReadCache == 1)
    {
        // free_cache.insert(chunk_map[key].offset_cache,chunk_map[key].offset_cache/CHUNK_SIZE);
